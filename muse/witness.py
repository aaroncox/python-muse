...
class Witness(dict):
    ...
    def refresh(self):
        ...
        if len(parts) == 3:
            ...
            if int(b) == 6:
                witness = self.muse.rpc.get_object(self.witness)
            else:
                witness = self.muse.rpc.get_witness_by_account(self.witness)
        else:
            account = Account(self.witness)
            witness = self.muse.rpc.get_witness_by_account(account["id"])
        if not witness:
...
class Witnesses(list):
    ...
    def __init__(self, muse_instance=None):
        self.muse = muse_instance or shared_muse_instance()
        self.schedule = self.muse.rpc.get_object("2.12.0").get("current_shuffled_witnesses", [])
