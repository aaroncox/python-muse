...
class Asset(dict):
    ...
    def refresh(self):
        ...
        """
        from musebase import asset_permissions

        asset = self.muse.rpc.get_asset(self.asset)
        if not asset:
            raise AssetDoesNotExistsException
        super(Asset, self).__init__(asset)
        if self.full:
            if self.is_bitasset:
                self["bitasset_data"] = self.muse.rpc.get_object(asset["bitasset_data_id"])
            self["dynamic_asset_data"] = self.muse.rpc.get_object(asset["dynamic_asset_data_id"])
