...
class Account(dict):
    """ This class allows to easily access Account data
        ...
        :rtype: dictionary
        :raises muse.exceptions.AccountDoesNotExistsException: if account does not exist
    ...
    def refresh(self):
        ...
        if re.match("^1\.2\.[0-9]*$", self.name):
            account = self.muse.rpc.get_objects([self.name])[0]
        else:
            account = self.muse.rpc.lookup_account_names([self.name])[0]
        if not account:
            raise AccountDoesNotExistsException(self.name)

        if self.full:
            account = self.muse.rpc.get_full_accounts([account["id"]], False)[0][1]
    ...
    def balances(self):
        """ List balances of an account. This call returns instances of
            :class:`muse.amount.Amount`.
        """
        balances = self.muse.rpc.get_account_balances(self["id"], [])
        return [
            Amount(b, muse_instance=self.muse)
    ...
    def history(
        ...
        mostrecent = self.muse.rpc.get_account_history(
            self["id"],
            "1.11.{}".format(0),
            1,
        ...
        while True:
            # RPC call
            txs = self.muse.rpc.get_account_history(
                self["id"],
                "1.11.{}".format(last),
                _limit,
                "1.11.{}".format(first - 1),
    ...
    def upgrade(self):
        return self.muse.upgrade_account(account=self)


class AccountUpdate(dict):
    """ This purpose of this class is to keep track of account updates
        as they are pushed through by :class:`muse.notify.Notify`.
    ...
    def account(self):
        """ In oder to obtain the actual
            :class:`muse.account.Account` from this class, you can
