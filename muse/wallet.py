...
from musebase.account import PrivateKey, GPHPrivateKey
...
class Wallet():
    """ The wallet is meant to maintain access to private keys for
        ...
        :param MuseNodeRPC rpc: RPC connection to a muse node
        :param array,dict,string keys: Predefine the wif keys to shortcut the wallet database

        Three wallet operation modes are possible:

        * **Wallet Database**: Here, pymuse loads the keys from the
    ...
    def setKeys(self, loadkeys):
        """ This method is strictly only for in memory keys that are
            passed to Wallet/muse with the ``keys`` argument
        """
    ...
    def addPrivateKey(self, wif):
        ...
        """
        # it could be either graphenebase or musebase so we can't check the type directly
