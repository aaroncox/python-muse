...
def history(ctx, account, limit, type, csv, exclude, raw):
    ...
    """
    from musebase.operations import getOperationNameForId
    ...
    for a in account:
        account = Account(a, muse_instance=ctx.muse)
...
def newaccount(ctx, accountname, account, password):
    ...
    """
    pprint(ctx.muse.create_account(
        accountname,
        registrar=account,
        password=password,
    ))
...
def upgrade(ctx, account):
    pprint(ctx.muse.upgrade_account(account))
